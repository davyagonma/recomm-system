#!/usr/bin/env python3
"""
Compare CSP+FC without business constraints to classic FC.

Classic baseline: Top-N on R_hat = R @ W_FC.
...
def run_comparison(
    ...
    """
    Compare CSP+FC against classic Top-N on the same R_hat and candidate sets.

    Both approaches use cardinality only; no business constraints are applied.
    """
    pipe, t_pipe = timed_call(
        build_pipeline, R, method="fc", n_candidates=n_candidates
    )
...
def main() -> None:
    """CLI entry point for FC comparison without business constraints."""
    ...
    parser.add_argument(
        "--use-memoire-matrix",
        action="store_true",
        help="Use the 4x4 toy matrix from the project README instead of 5x5",
    )
    ...
    print(f"Parameters: method=fc, slate_size={args.slate_size}")

    result = run_comparison(R, slate_size=args.slate_size, n_candidates=args.n_candidates)

    json_path = save_json(result, "compare_fc_sans_contraintes.json")
    csv_rows = result["comparison"]["per_user"]
    csv_path = save_csv_table(csv_rows, "compare_fc_sans_contraintes.csv")
